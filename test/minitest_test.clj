...
(deftest test-on-load
  ...
  (let [printed (with-contexts {:exec-mode :load}
                  (with-out-str (require 'minitest-test-namespace :reload) #_(load "minitest_test_namespace")))]
    (testing "tests are stored"
      (is (= 5 (count (apply concat (get @*tests* 'minitest-test-namespace))))))
