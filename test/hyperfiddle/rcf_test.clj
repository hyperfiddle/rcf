  ...
  (testing "`tests` behaves like"
    (testing "`cc/comment` when RCF is disabled."
      (binding [rcf/*enabled* false]
        (is (nil? (rcf/tests* '(1 := 1))))))
    (testing "`cc/do` when RCF is enabled."
      (is (= '(do) (macroexpand-1 '(tests))))
      (is (= '1 (macroexpand-1 '(tests 1))))
      (is (= '(do 1 2) (macroexpand-1 '(tests 1 2)))))))

(deftest nesting
  (testing "Nested `tests` flattens"
    (is (= '(do) (macroexpand-1 '(tests (tests)))))
    (is (= '(do 1 2) (macroexpand-1 '(tests 1 (tests 2)))))))

(deftest documentation-behavior
