...
(alter-meta! #'unify assoc :doc
             (str (:doc (meta #'garner-unifiers))
                  "  Note: This function is implemented with an occurs-check."))

(def ^{:arglists '([expression bindings])}
  subst   (make-occurs-subst-fn lvar?))
(alter-meta! #'subst assoc :doc (:doc (meta #'try-subst)))

(def ^{:arglists '([expression1 expression2])}
  unifier (make-occurs-unifier-fn lvar?))
(alter-meta! #'unifier assoc :doc
             (str (:doc (meta #'unifier*))
                  "  Note: This function is implemented with an occurs-check."))
