...
(defn- map-entry-proxy [m k]
  (macros/case
    :clj  (reify clojure.lang.IMapEntry (key      [_] k)
                                        (val      [_] (.valAt m k))
                 java.util.Map$Entry    (getKey   [t] (.key t))
                                        (getValue [t] (.val t)))
    :cljs (reify IMapEntry    (-key     [_] k)
...
(comment
  (def-map-type FuncMap [m deep]
    (get [this k default]  (let [result (get m k default)
                                 result (if (-> result meta :minitest/map-func)
                                          (result this)
                                          result)]
                             (if (and (map? result)
                                      (not (instance? FuncMap result))
                                      deep)
                               (FuncMap. result deep)
                               result)))
    (assoc     [this k v]  (FuncMap. (assoc m k v)      deep))
    (dissoc    [this k]    (FuncMap. (dissoc m k)       deep))
    (keys      [this]      (keys m))
    (meta      [this]      (meta m))
    (with-meta [this meta] (FuncMap. (with-meta m meta) deep))))
